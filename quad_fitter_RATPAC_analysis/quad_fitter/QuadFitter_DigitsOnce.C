...
vector<QUAD> MakeQuadruplets(std::vector<int> vSeedDigitList){
  int ff=0;
  vec.clear();
  Int_t numEntries = (Int_t) vSeedDigitList.size();
  for(int j = 0; j < 100*NSeedsTarget; ++j ){
  ...
  //std::shuffle(std::begin(fLastEntry_v), std::end(fLastEntry_v), std::default_random_engine());
  //for(int u = 0; u < fLastEntry_v.size(); ++u ){ cout<<"fLastEntry_v[u] "<<fLastEntry_v[u]<<endl;}

  int nquad = fLastEntry_v.size()/4; //NSeedsTarget; //v.size()/4;
  //cout<<" posible #quads for this event: "<<fLastEntry_v.size()/4<<" out of "<<NSeedsTarget<<endl;
 
  //std::vector<QUAD> vec; vec.reserve(1000);
  QUAD thequad; 
  //if(NSeedsTarget>1000){std::cerr<<"vector size is 1000. Change the size or the number of quads."<<endl;}
  for (int i = 0; i <fLastEntry_v.size(); ++i ){
     if(i%4==0){
      for(int v=i; v<(i+4); ++v ){
       thequad.push_back(fLastEntry_v[v]); 
       //cout<<"this quad "<< v <<" has: "<<fLastEntry_v[v]<<endl;
      }
    /*auto start = fLastEntry_v.begin() + 3*i;
    auto stop = start + 3;
    QUAD thequad = QUAD(start, stop);*/
    //cout<<"thequad.size() "<<thequad.size()<<endl;
    //cout<<"i: "<<i<<" thequad[i] "<<thequad[i]<<endl;
    if(ff<nquad){ //cout<<"add the quad"<<endl;
      vec.push_back(thequad); ff++; 
      thequad.clear();  
    }
   }
  }
  ...
  /*for(int k = 0; k < vec.size(); ++k ){
    for(int l = 0; l < 4; ++l ){
    //cout<<"vec[k].size() "<<vec[k].size()<<endl;
    cout<<"k: "<<k<<" l "<<l<<" vec[k][l] "<<vec[k][l]<<endl;
  }}*/
 return vec;
}

std::vector<QUAD> quad_v;
...
Int_t ChooseNextDigitOnce(Double_t& xpos, Double_t& ypos, Double_t& zpos, Double_t& time, std::vector<double> fDigitX,std::vector<double> fDigitY,std::vector<double> fDigitZ,std::vector<double> fDigitT, std::vector<int> vSeedDigitList){
  ...
  */
   if(countQ==0){ 
     quad_v = MakeQuadruplets(vSeedDigitList);  
     //cout<<"quad_v.size(): "<<quad_v.size()<<endl;
   }
   if(countQ < quad_v.size()){
     //cout<<"countQ: "<<countQ<<" countD "<<countD<<" quad_v[countQ][countD] "<<quad_v[countQ][countD]<<endl;
     fLastEntry_once=quad_v[countQ][countD];
   }
   countD++;
   if(countD>3){
      countQ++;
...
std::vector<vector<double>> CalcVertexSeeds(std::vector<double> fDigitX,std::vector<double> fDigitY,std::vector<double> fDigitZ,std::vector<double> fDigitT,std::vector<double> fDigitQ,std::vector<double> fDigitW,std::vector<double> fDigitV,std::vector<int> vSeedDigitList)
{
    ...
    while( vSeedVtxX.size()<NSeedsTarget && (countQ<quad_v.size()+1) )//&& counter<NSeedsTarget && counter0<NSeedsTarget)
    {
...
void QuadFitter_DigitsOnce(const char* filename_ratpac) {
    ...
    for (sub_event = 0; sub_event < ds->GetEVCount(); sub_event++) {
      ...
      for (hit = count = veto_count = 0; hit < nhit; hit++) {
        ...
        if (pmtinfo->GetType(id) == innerPMTcode) {
          ...
          cout<<"Hit: "<<hit<<"|"<<count<<" PMTid "<<cables[count]<<" hitpos "<<hitpos_x[count]<<","<<hitpos_y[count]<<","<<hitpos_z[count]<<" time: "<<times[count]<<" charge: "<<charges[count]<<" fThisDigit "<<fThisDigit<<endl;	  
