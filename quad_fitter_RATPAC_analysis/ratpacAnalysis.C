...
//Edited by: E. Drakopoulou, April 2021
// To run in root:
// .x ratpacAnalysis("infile.root") 
// (where infile.root is raw rat output)
// if you undo the comments you can add the outfile.root outfile.csv as outputs
...
*/
int ratpacAnalysis(const char* filename_ratpac) {
  ...
  for (event = 0; event < n_events; event++)
    {
      ...
      for(sub_event=0;sub_event<ds->GetEVCount();sub_event++)
        ...
        cout<<"mc: trueEnergy: "<<trueEnergy<<" vertex: "<<trueX<<","<<trueY<<","<<trueZ<<" time: "<<trueT<<endl;
        if (subid>0)
          {
        ...
        for(hit=innerHit=vetoHit=0; hit<nhit; hit++)
          ...
          if(pmtinfo->GetType(id) == innerPMTcode)
            ...
            cout<<"hits position: "<<pos[0]*0.1<<","<<pos[1]*0.1<<","<<pos[2]*0.1<<" time: "<<pmt->GetTime()<<" charge: "<<pmt->GetCharge()<<endl;
